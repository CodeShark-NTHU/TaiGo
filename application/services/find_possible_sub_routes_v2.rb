  ...
  class FindPossibleSubRoutesV2
    ...
    def get_stops_near_both_start_dest(input)
      ...
      s_piars_of_stopdis.map do |pair|
        ...
        sors.map do |sor|
          if sor.stop.name.chinese == "新竹高中"
            result_start << [sor.sub_route_id, stop, distance]
            puts "start: #{sor.sub_route.name.chinese} #{sor.sub_route.headsign} #{sor.stop.name.chinese} #{sor.stop.name.english}"
          end
        end
      end
=begin
...
=end
      ...
      d_piars_of_stopdis.map do |pair|
        ...
        sors = Repository::For[Entity::StopOfRoute].find_stop_id(stop.id)
        sors.map do |sor|
          if sor.stop.name.chinese == "水源地"
            result_dest << [sor.sub_route_id, stop, distance]
            puts "dest: #{sor.sub_route.name.chinese} #{sor.sub_route.headsign} #{sor.stop.name.chinese} #{sor.stop.name.english}"
          end
        end
      end
=begin
      ...
      start_set.each do |s_item|
        dest_set.each do |d_item|
          if d_item[0] == s_item[0]
            match_set << [s_item[0], s_item[1], d_item[1], s_item[2]]
          end
        end
      end
      match_sort = {}
